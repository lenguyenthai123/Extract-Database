...
public class PdfExportServiceImpl implements ExportService {
    ...
    public byte[] exportDatabase(RequestDto requestDto) {
        ...
        try (ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream()) {
            ...
            Paragraph title = new Paragraph("Database: " + database.getName()).setFont(font).setFontSize(PdfGenerator.getFontSizeForTitleLevel(1)).setTextAlignment(TextAlignment.CENTER).setBold();
            ...
            if (database.getSchemas().isEmpty()) {
                ...
                document.add(new Paragraph("\nNo data found").setFont(font).setFontSize(PdfGenerator.getFontSizeForTitleLevel(4)).setTextAlignment(TextAlignment.CENTER));
            }
